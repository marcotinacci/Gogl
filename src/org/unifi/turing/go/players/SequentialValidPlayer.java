...
public class SequentialValidPlayer extends AbstractPlayer {
	private int[][] board;
	private int myID;
	private Goban goban;

	public SequentialValidPlayer() {
	...
	public int[] nextStone() {
		...
		for (int r = 0; r < n; r++) {
			for (int c = 0; c < n; c++) {
				if (board[r][c] == 0 && !goban.isSuicide(stone, r, c)) {
					board[r][c] = myID;
					goban.putStoneAt(stone, r, c);
					return new int[] { r, c };
				}
			}
		}
